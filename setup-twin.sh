...
curl -fsSL "${FRAMEWORK_BASE}/CHIMERA-The-Complete-Book-v12.5.md" -o framework/BOOK.md 2>/dev/null || echo "  (Book download skipped — available at chimera-framework repo)"
curl -fsSL "${FRAMEWORK_BASE}/archive/CHIMERA-BODY-SCAN-PROTOCOL-v3.6.md" -o framework/PROTOCOL.md 2>/dev/null || echo "  (Protocol download skipped)"
...
curl -fsSL "${FRAMEWORK_BASE}/CHIMERA-The-Complete-Book-v12.5.md" -o framework/BOOK.md 2>/dev/null && echo "  Book updated" || echo "  Book: no changes"
curl -fsSL "${FRAMEWORK_BASE}/archive/CHIMERA-BODY-SCAN-PROTOCOL-v3.6.md" -o framework/PROTOCOL.md 2>/dev/null && echo "  Protocol updated" || echo "  Protocol: no changes"
...
git add .
git commit -m "Initial twin setup for ${PERSON_NAME} — CHIMERA v12.5"
git push
