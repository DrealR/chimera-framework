...
| [Attention's Three Properties](docs/attention-three-properties.md) | The attention organ the Prime trains has three scannable axes: direction (outward/inward), capture-resistance (can something seize it), and occupancy-tolerance (can you sit with the unfilled vacuum). Names engineered occupancy (the always-on environment eroding tolerance by design), the meta-observer's cost (novelty-depletion at high resolution), and modulation — the Prime gains an intensity dial, not just a steering wheel |
