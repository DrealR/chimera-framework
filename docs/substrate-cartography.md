...
- [The Deletion Test](the-deletion-test.md) — the private/public register (§8) is the boundary the deletion test relies on: failed body-scan passes stay in the private register before any graduation decision; cartography makes that boundary explicit and enforceable so the record of failures is kept without polluting the public surface.
- [Deployment Hygiene Discipline](deployment-hygiene-discipline.md) — knowing which surface lives on which platform, in which repo, behind which alias is the prerequisite for verifying deploys; cartography is the prerequisite this discipline requires. Map first, deploy second.
