...
- [Scaffolding as Substance](scaffolding-as-substance.md) — the generate-abundance-then-trim build discipline; each generate→trim cycle maps to one QB micro-breath (inhale = generate, pause+exhale = trim), and the 15-min/50-line thresholds bound each cycle, making the subtractive model structurally safe.
- [Deployment Hygiene Discipline](deployment-hygiene-discipline.md) — the ship-cycle complement: micro-commits ensure code reaches the repo; this discipline ensures the repo reaches production, end-to-end. Pair with QB Protocol for tight, observable deploy chains.
